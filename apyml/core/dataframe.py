...
class Handler(object):
    ...
    def read_from_src(self, path: str, typ: str, extension: str, **kwargs) -> pandas.DataFrame:
        return {
            'File': self._fileHandler,
            'S3': self._s3Handler
        }.get(typ, None)(path, extension, **kwargs)

    def _fileHandler(self, path: str, extension: str, **kwargs) -> pandas.DataFrame:
    ...
    def _s3Handler(self, path: str, extension: str, **kwargs) -> pandas.DataFrame:
        return None

def create_dataframe_from_src(path: str, typ: str, extension: str, **kwargs) -> pandas.DataFrame:
    from apyml.context import Context
    context = Context()

    readers_opts = context.get_config('readers_opts')
    handler = Handler(readers_opts)
    return handler.read_from_src(path, typ, extension, **kwargs)